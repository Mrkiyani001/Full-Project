...
window.showConfirmModal = function (message, onConfirm) {
    ...
    overlay.className = 'fixed inset-0 z-[10000] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in p-4';

    overlay.innerHTML = `
        <div class="bg-[#1e2330] border border-white/10 rounded-xl p-6 max-w-sm w-full shadow-2xl transform scale-100 transition-all" onclick="event.stopPropagation()">
            <h3 class="text-white font-bold text-lg mb-2">Confirm Action</h3>
            <p class="text-slate-300 text-sm mb-6">${message}</p>
            <div class="flex justify-end gap-3">
                <button id="confirm-cancel-btn" class="text-slate-400 hover:text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-white/5 transition-colors">Cancel</button>
                <button id="confirm-action-btn" class="bg-red-500 hover:bg-red-600 text-white text-sm font-bold px-4 py-2 rounded-lg shadow-lg shadow-red-500/20 transition-all hover:scale-105 active:scale-95">Delete</button>
            </div>
        </div>
    `;
    ...
    overlay.onclick = close; // Click outside to close
...
window.canDeleteContent = function (currentUser, ownerUser) {
    if (!currentUser || !ownerUser) return false;
    if (currentUser.id === ownerUser.id) return true;

    if (currentUser.roles) {
        // Robust check for string array or object array
        const roles = currentUser.roles.map(r => (typeof r === 'object' ? r.name : r).toLowerCase());
...
// Duplicate createPostHTML removed. Using the one defined later in file.
...
window.hasRole = (user, roleName) => {
    ...
    if (user.roles && Array.isArray(user.roles)) {
        return user.roles.some(r => {
            const rName = (typeof r === 'object' && r.name) ? r.name : r;
            return String(rName).toLowerCase() === String(roleName).toLowerCase();
        });
    }
    return false;
};
            ...
            <div class="flex gap-3 items-end">
                    ...
                    <div class="flex-1 bg-white/5 border border-white/10 rounded-2xl flex items-end gap-2 p-1.5 focus-within:border-primary/50 transition-colors">
                        <textarea id="comment-input-${post.id}" 
                            class="bg-transparent border-none text-slate-200 text-sm placeholder:text-slate-500 flex-1 min-h-[36px] max-h-[120px] py-2 px-3 resize-none focus:ring-0 leading-relaxed custom-scrollbar" 
                            placeholder="Write a comment..." 
                            rows="1"
                            oninput="this.style.height = 'auto'; this.style.height = this.scrollHeight + 'px'"></textarea>
                        
                        <div class="flex items-center gap-1 pb-1 pr-1">
                             <label for="comment-file-${post.id}" class="cursor-pointer text-slate-400 hover:text-white transition-colors p-2 rounded-full hover:bg-white/10 shrink-0">
                            ...
                            <button onclick="submitComment(${post.id})" class="p-2 bg-primary text-white rounded-xl hover:bg-blue-600 transition-colors shadow-lg shadow-blue-500/20 shrink-0 flex items-center justify-center">
                                <span class="material-symbols-outlined text-[20px]">send</span>
                            </button>
                        </div>
                    </div>
            </div>
            <div id="comment-preview-${post.id}" class="mt-2 ml-11 hidden"></div>
        </div>

        <!-- Hidden Comment List Section -->
        <div id="comment-section-${post.id}" class="hidden px-4 pb-4 border-t0 border-white/5 bg-transparent">
            <div id="comment-list-${post.id}" class="flex flex-col gap-3 py-4 max-h-80 overflow-y-auto pr-2 custom-scrollbar"></div>
        </div>
    </article>`;
}
                ...
                list.innerHTML = data.data.items.map(comment => {
                    ...
                    if (comment.replies && comment.replies.length > 0) {
                        repliesHTML += comment.replies.map(reply => {
                            ...
                            const canDelete = window.canDeleteContent(currentUser, replyUser);

                            // Explicitly check for Admin/Super Admin as requested
                            const isAdmin = window.hasRole(currentUser, 'admin') || window.hasRole(currentUser, 'super admin');

                            console.warn(`Reply ${reply.id} Debug:`, { isReplyOwner, canDelete, isAdmin, currentUserRoles: currentUser.roles });
                            ...
                            // Delete: Owner OR Admin OR Super Admin
                            if (isReplyOwner || canDelete || isAdmin) {
                                menuItems += `
                                ...
                                if (['image', 'jpg', 'png', 'jpeg', 'gif'].includes(att.file_type) || /\.(jpg|jpeg|png|gif)$/i.test(path)) {
                                    ...
                                    return `<a href="${path}" target="_blank" class="flex items-center gap-2 p-2 bg-white/5 rounded-lg border border-white/10 hover:bg-white/10 transition-colors"><span class="material-symbols-outlined text-slate-400">description</span><span class="text-xs text-blue-400 underline">View Attachment</span></a>`;
                                        ...
                                        <div class="flex items-center gap-3 mt-1 ml-2">
                                            ...
                                            <!-- Inline Actions -->
                                            ${isReplyOwner ? `
                                            <button onclick="editPostReply(${reply.id})" class="flex items-center gap-0.5 text-[10px] font-medium text-slate-400 hover:text-white transition-colors">
                                                <span class="material-symbols-outlined text-[14px]">edit</span>
                                                <span>Edit</span>
                                            </button>` : ''}

                                            ${(isReplyOwner || canDelete || isAdmin) ? `
                                            <button onclick="deletePostReply(${reply.id}, this)" class="flex items-center gap-0.5 text-[10px] font-medium text-slate-400 hover:text-red-400 transition-colors">
                                                <span class="material-symbols-outlined text-[14px]">delete</span>
                                                <span>Delete</span>
                                            </button>` : ''}

                                            ${!isReplyOwner ? `
                                            <button onclick="openReportModal(${reply.id}, 'reply')" class="flex items-center gap-0.5 text-[10px] font-medium text-slate-400 hover:text-yellow-500 transition-colors">
                                        ...
                                        <div class="flex justify-between items-start mb-1">
                                            ...
                                            <div class="relative group">
                                                ...
                                                <div class="post-comment-menu absolute right-0 top-6 w-32 bg-[#1e2330] border border-white/10 rounded-lg shadow-xl py-1 z-20 opacity-0 scale-95 pointer-events-none transition-all duration-200 origin-top-right">
                                                    ...
                                                    ${window.canDeleteContent(currentUser, user) ? `
...
async function submitComment(postId) {
    ...
    try {
        ...
        if (data.success) {
            ...
            // Update UI count
            // Use window.updatePostCommentCountUI for safety if defined locally or globally
            if (typeof updatePostCommentCountUI === 'function') {
                updatePostCommentCountUI(input, 1);
            } else if (typeof window.updatePostCommentCountUI === 'function') {
                window.updatePostCommentCountUI(input, 1);
            }

            toggleCommentSection(postId); // Refresh list
...
async function openUserListModal(arg1, arg2) {
    ...
    if (!modal) {
        document.body.insertAdjacentHTML('beforeend', `
        <div id="likers-modal" class="fixed inset-0 z-[70] hidden transition-all duration-200">
            <div class="absolute inset-0 bg-black/80 backdrop-blur-sm opacity-0 transition-opacity duration-200" onclick="closeLikersModal()"></div>
            <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm bg-[#1e2330] border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh] scale-95 opacity-0 transition-all duration-200">
                <div class="p-4 border-b border-white/10 flex justify-between items-center bg-[#1e2330]">
                    <h3 class="text-white font-bold">Likes</h3>
                    <button onclick="closeLikersModal()" class="text-slate-400 hover:text-white"><span class="material-symbols-outlined">close</span></button>
                </div>
                <div id="likers-list" class="overflow-y-auto p-2 custom-scrollbar flex-1">
                    <div class="text-center text-slate-500 py-8">Loading...</div>
                </div>
            </div>
        </div>`);
        modal = document.getElementById('likers-modal');
    }

    const list = document.getElementById('likers-list');

    modal.classList.remove('hidden');
    // Minimal delay to allow display block to take effect before opacity transition
    ...
    list.innerHTML = '<div class="text-center text-slate-500 py-8"><span class="material-symbols-outlined animate-spin text-2xl">progress_activity</span></div>';
    ...
    if (titleEl) {
        if (type === 'post') titleEl.textContent = 'Post Likes';
        else if (type === 'comment') titleEl.textContent = 'Comment Likes';
        else if (type === 'reply') titleEl.textContent = 'Reply Likes';
        else if (type === 'followers') titleEl.textContent = 'Followers';
        else if (type === 'following') titleEl.textContent = 'Following';
    }
    ...
    if (type === 'comment') {
        ...
        endpoint = '/fetch_followers';
        body = { user_id: id };
    } else if (type === 'following') {
        endpoint = '/fetch_following';
        body = { user_id: id };
    }
    ...
    try {
        ...
        if (data.success) {
             // Handle both array (fetch_followers) and paginated object (fetch_following)
             const items = Array.isArray(data.data) ? data.data : (data.data.items || []);
             renderUserList(items, list);
        } else {
...
window.updatePostCommentCountUI = function (elementWithinPost, delta) {
    // Try to find the comment section first, as it's a reliable anchor
    const section = elementWithinPost.closest('[id^="comment-section-"]');
    let postId = null;
    let postArticle = null;

    if (section) {
        postId = section.id.replace('comment-section-', '');
        postArticle = document.getElementById(`post-${postId}`);
    } else {
        // Fallback: try to find closest article if we are not in the section (e.g. maybe deleting from some other view?)
        let article = elementWithinPost.closest('article');
    ...
    if (!postArticle || !postId) return;

    // Find the comment button within this specific post article
    // We use a looser selector to avoid spacing mismatches
    let btn = postArticle.querySelector('button[onclick*="toggleCommentSection"]');
    ...
    if (btn) {
        ...
        if (countSpan) {
            ...
            if (isNaN(count) || text.toLowerCase() === 'comment') count = 0;
...
window.deletePostComment = function (commentId, btn) {
    window.closeAllPostMenus();
...
window.deletePostReply = function (replyId, btn) {
    window.closeAllPostMenus();
    showConfirmModal("Delete this reply?", async () => {
        updatePostCommentCountUI(btn, -1);
        ...
        try {
            ...
            if (data.success) {
                ...
                if (typeof toggleCommentSection === 'function') {
                    // Ensure list is OPEN and refreshed
                    const section = document.getElementById(`comment-section-${postId}`);
                    if (section) {
                        if (section.classList.contains('hidden')) {
                            toggleCommentSection(postId); // Open it
                        } else {
                            // It's already open, so we want to refresh it.
                            // toggleCommentSection toggles it closed, which we don't want.
                            // So we toggle (close) then toggle (open).
                    ...
                    list.innerHTML = data.data.items.map(comment => {
                        ...
                        if (comment.replies && comment.replies.length > 0) {
                            repliesHTML += comment.replies.map(reply => {
                                ...
                                console.warn(`Reply ${reply.id} Debug (Toggle):`, { isReplyOwner, canDelete, isAdmin, currentUserRoles: currentUser.roles });
    ...
    window.submitReply = async function (commentId) {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Find the post ID from the DOM structure
                const inputEl = document.getElementById(`reply-input-${commentId}`);
                if (inputEl) {
                    // The input is inside #comment-list-{postId} -> ... -> #reply-box-{commentId}
                    // We can traverse up to find the comment-section or comment-list
                    const listContainer = inputEl.closest('[id^="comment-list-"]');
                    if (listContainer) {
                        const postIdPart = listContainer.id.replace('comment-list-', '');
                        const postId = parseInt(postIdPart);
                        if (postId && typeof toggleCommentSection === 'function') {
                            // Force refresh
                            toggleCommentSection(postId);
                            setTimeout(() => toggleCommentSection(postId), 100);
                        }
